...
def parse_spor_by_fam(virhost:str,
    ...
    vh_data.index = vh_data.index.str.replace('.fa', '')
    ...
    # add to summary
    summary_info['Number of unique mags in VirHost file'] = \
       len(vh_data.index.unique())
    summary_info['Number of unique mags in taxonomy file'] = \
       len(tx_data.index.unique())
    summary_info['Number of unique mags in both VirHost and taxonomy'] = \
        len(data.index.unique())
    summary_info["Number of VirHost mags dropped because there is"
                 " no match in taxonomy"] = \
    ...
    summary_info["The number of observations removed by the value filter"] = \
        pre_filter_len - len(output)

    # print summary
    for i, j in summary_info.items():
        print(f"{i}: {j}")
